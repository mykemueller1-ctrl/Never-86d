...
export default function StationBroadcastScreen({ onBack, staffUser }: StationBroadcastScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      {/* Header */}
      <div className="bg-gradient-to-r from-red-900/80 to-orange-900/80 border-b border-slate-200">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-500">86'd items, alerts, cross-station comms</p>
          </div>
        </div>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition-colors flex items-center justify-center gap-1.5 ${
              tab === t.key ? 'text-red-400 border-b-2 border-red-400' : 'text-slate-400'
            }`}
          >
            {t.label}
            {'count' in t && (t.count ?? 0) > 0 && (
              <span className="bg-red-500 text-slate-900 text-[10px] px-1.5 py-0.5 rounded-full font-bold">{t.count}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="px-4 py-4">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              <button
                ...
                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                  !stationFilter ? 'bg-red-600 text-slate-900' : 'bg-white/5 text-slate-400'
                }`}
              >
                All
              </button>
              {STATIONS.map(s => (
                <button
                  ...
                  className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${
                    stationFilter === s ? 'bg-red-600 text-slate-900' : 'bg-white/5 text-slate-400'
                  }`}
                >
                  {s}
                </button>
                    ...
                    <div className="px-4 py-3">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {b.message && <p className="text-xs text-slate-500 mt-1">{b.message}</p>}
                          <div className="flex items-center gap-2 mt-2 text-[10px] text-slate-900/30">
                          ...
                          {b.createdByName && (
                            <div className="text-[10px] text-slate-900/20 mt-1">By: {b.createdByName}</div>
                          )}
                        </div>
                      </div>
                      ...
                      {acknowledged.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {acknowledged.map((ack: any, i: number) => (
                            <span key={i} className="text-[10px] bg-white/5 px-2 py-0.5 rounded-full text-slate-400">
        ...
        {tab === 'create' && (
          <div className="space-y-4">
            ...
            {/* Item Name */}
            <div>
              <label className="text-xs font-semibold text-slate-400 mb-2 block">Item Name</label>
              <input
            ...
            {/* Message */}
            <div>
              <label className="text-xs font-semibold text-slate-400 mb-2 block">Message (optional)</label>
              <textarea
                className="w-full bg-white/5 border border-slate-200 rounded-lg px-3 py-2.5 text-sm resize-none"
                rows={2}
            ...
            {/* From Station */}
            <div>
              <label className="text-xs font-semibold text-slate-400 mb-2 block">From Station</label>
                ...
                {STATIONS.map(s => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${
                      fromStation === s ? 'bg-indigo-600 text-slate-900' : 'bg-white/5 text-slate-400'
                    }`}
                  >
                    {s}
                  </button>
            ...
            {/* Target Stations */}
            <div>
              <label className="text-xs font-semibold text-slate-400 mb-2 block">Notify Stations</label>
              <div className="flex gap-2 flex-wrap">
                {STATIONS.filter(s => s !== fromStation).map(s => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${
                      targetStations.includes(s) ? 'bg-red-600 text-slate-900' : 'bg-white/5 text-slate-400'
                    }`}
                  >
        ...
        {tab === 'history' && (
          <div className="space-y-2">
            {broadcastHistory.isLoading ? (
              ...
              <div className="text-center py-10 text-slate-900/30 text-sm">No broadcast history</div>
            ) : (
              broadcastHistory.data?.map((b: any) => {
                const typeConfig = getTypeConfig(b.broadcastType);
                return (
                  <div key={b.id} className="bg-white/5 rounded-xl px-4 py-3 border border-slate-200 flex items-center gap-3">
