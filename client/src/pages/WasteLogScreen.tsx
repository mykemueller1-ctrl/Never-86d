...
const WASTE_TYPES = [
  ...
  { value: 'other', label: 'Other', color: 'bg-white/5 text-slate-500 border-slate-200' },
];
...
export default function WasteLogScreen({ onBack, staffUser }: WasteLogScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      {/* Header */}
      <div className="bg-gradient-to-r from-red-900/80 to-pink-900/80 border-b border-slate-200">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-500">Log waste, track patterns, reduce costs</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-200">
        {[
          ...
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === t.key ? 'text-red-400 border-b-2 border-red-400' : 'text-slate-400'}`}
          >
            {t.label}
          </button>
        ))}
      ...
      <div className="px-4 py-4">
        {tab === 'report' && (
          <div className="space-y-4">
            ...
            {/* Waste Type */}
            <div>
              <label className="text-xs font-semibold text-slate-400 mb-2 block">Waste Type</label>
                ...
                {WASTE_TYPES.map(t => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-xs font-medium border transition-all ${
                      wasteType === t.value ? t.color : 'bg-white/5 text-slate-900/30 border-slate-200'
                    }`}
                  >
                    {t.label}
                  </button>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-medium border transition-all ${
                  preventable ? 'bg-red-500/10 text-red-300 border-red-500/20' : 'bg-white/5 text-slate-900/30 border-slate-200'
                }`}
              >
          ...
          <div className="space-y-3">
              ...
              {[1, 7, 14, 30].map(d => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                    days === d ? 'bg-red-600 text-slate-900' : 'bg-white/5 text-slate-400'
                  }`}
                >
                  {d === 1 ? 'Today' : `${d}d`}
                </button>
            ...
            {wasteLog.isLoading ? (
              ...
              <div className="text-center py-10">
                <Trash2 className="w-12 h-12 text-slate-900/10 mx-auto mb-3" />
                <p className="text-slate-900/30 text-sm">No waste logged in this period</p>
              </div>
            ) : (
              wasteLog.data?.map((w: any) => {
                const typeConfig = getWasteTypeConfig(w.wasteType);
                return (
                  <div key={w.id} className="bg-white/5 rounded-xl px-4 py-3 border border-slate-200">
                    <div className="flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[10px] text-slate-900/30">{w.quantity} {w.unitOfMeasure}</span>
                          {w.preventable && (
                        ...
                        {w.reason && <p className="text-[10px] text-slate-900/30 mt-1">{w.reason}</p>}
                      </div>
                      ...
                      {((wasteSummary.data as any).byType as any[]).map((t: any, i: number) => {
                        ...
                        return (
                          <div key={i} className="flex items-center justify-between bg-white/5 rounded-lg px-3 py-2">
                            ...
                            <div className="flex items-center gap-3">
                              <span className="text-xs text-slate-400">{t.count} entries</span>
